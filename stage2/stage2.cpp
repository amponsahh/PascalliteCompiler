...
void CreateListingHeader() {
  ...
  listingFile << "STAGE2:" << right << setw(29) << names << right << setw(32) << DATE << endl;
...
void PrintSymbolTable() {
  ...
  time_t now = time(NULL);
  string DATE = ctime( & now);
  cout << "STAGE2:" << right << setw(29) << names << right << setw(32) << DATE << endl;
...
// the next token / end of file marker
string NextToken() {
  token = "";
  while (token == "") {
    if (charac == '{') {
...
void ExecStmts() {
  NextToken();
  while (isNON_KEY_ID(token) || token == "read" || 
         token == "write" || token == "if" || 
...
// TODO: fact check with draft - DONE Nana
void ExecStmt() {
  if (isNON_KEY_ID(token))
	AssignStmt();
  else if (token == "read")
	...
	throw "non-keyword identifier, \"read\", \"write\", \"if\", \"while\", \"repeat\", \";\" or \"begin\"";
}
...
// push name onto operandStk  **TODO alter changes - Done, Nana
void PushOperand(string name) {
  ...
  operandStk.push((name[0] == 'L') ? name : externalToInternal(name));
}
...
// TODO check diction and sync file names from here now - Done, Nana
void Code(string operatorr, string operand1, string operand2) {
  ...
  } else if (operatorr == "then") {
	EmitThenCode(operand1);
  } else if (operatorr == "else") {
	EmitElseCode(operand1);
  } else if (operatorr == "post_if") {
	EmitPostIfCode(operand1);
  } else if (operatorr == "while") {
	EmitWhileCode();
  } else if (operatorr == "do") {
	EmitDoCode(operand1);
  } else if (operatorr == "post_while") {
	EmitPostWhileCode(operand1, operand2);
  } else if (operatorr == "repeat") {
	EmitRepeatCode(); 
  } else if (operatorr == "until") {
...
void Expresses() {
  if (token == ")" || token == ";" || token == "do" || token == "then")
    return;
  if (isRelOp(token)) {
	  PushOperator(token);
	  NextToken();
...
// TODO: check syntax chart from here now on - Done, Nana
void IfStmt() {
  if (token != "if")
     throw "\"if\" expected";
  Express();
...
void ElsePt() {
   ...
   if (token == "end" || isNON_KEY_ID(token) || 
       token == ";" || token == "until" || 
       token == "begin" || token == "while" || 
   ...
   else throw "\"else\", \"end\", \";\", \"until\", \"begin\", \"while\", \"if\", \"repeat\", \"read\", \"write\", or NON-KEY-ID expected.";
}
...
void EmitThenCode(string operand) {
   if (symbolTable.at(getIntSymTableIndex(operand)).dataType != BOOLEAN)
      throw "boolean expression expected";
   string tempLabel = "L" + to_string(numLabels++);
   if (aReg != operand)
      objFileWrite("", "LDA", operand, "", "", "");
   string remarks = "if false jump to " + tempLabel;
   objFileWrite("", "AZJ", tempLabel, "", "", remarks);
...
void EmitElseCode(string operand) {
   string tempLabel = "L" + to_string(numLabels++);
   objFileWrite("", "UNJ", tempLabel, "", "", "jump to end if");
   objFileWrite(operand, "NOP", "", "", "", "else");
   PushOperand(tempLabel);
   aReg = "";
}

void EmitPostIfCode(string operand) {
   objFileWrite(operand, "NOP", "", "", "", "end if");
   aReg = "";
}
...
void EmitWhileCode() {
   string tempLabel = "L" + to_string(numLabels++);
   objFileWrite(tempLabel, "NOP", "", "", "", "while");
   PushOperand(tempLabel);
   aReg = "";
}
...
void EmitPostWhileCode(string operand1,string operand2) {
   objFileWrite("", "UNJ", operand2, "", "", "end while");
   objFileWrite(operand1, "NOP", "", "", "", "");
   aReg = "";
}

void EmitRepeatCode() {
   string tempLabel = "L" + to_string(numLabels++);
   objFileWrite(tempLabel, "NOP", "", "", "", "repeat");
   PushOperand(tempLabel);
   aReg = "";
}

void EmitUntilCode(string operand1, string operand2) {
   if (symbolTable.at(getIntSymTableIndex(operand1)).dataType != BOOLEAN)
      throw "boolean expression expected";
   if (aReg != operand1)
      objFileWrite("", "LDA", operand1, "", "", "");
   objFileWrite("", "AZJ", operand2, "", "", "until");
   if (operand1[0] == 'T' && operand1 != "TRUE")
      FreeTemp();
   aReg = "";
}